...
angular.module('config').controller('ConfigController', ['$scope', '$location', '$state', 'Authentication', '$resource', 'Config', 'ConfigId',
	function ($scope, $location, $state, Authentication, $resource, Config, ConfigId) {
		...
		$scope.buscarDatosBD = function () {
			ConfigId.get(function (response) {
		...
		$scope.guardar = function () {
			$scope.VistaEstados = Object.create(Objeto_Estados);
			var configdb = new Config($scope.form);
		...
		$scope.resetConnection = function () {
			ConfigId.resetConnection({ dbId: 'default' },
				function (response) {
